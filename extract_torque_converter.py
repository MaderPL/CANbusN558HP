...
def extract(candump_path: str, output_path: str) -> None:
    ...
    latest = {"reinf": None, "engine_rpm": None, "turbine_rpm": None}
    ...
    with open(candump_path) as f:
        for line in f:
            ...
            elif frame_id == "1af" and len(hex_data) >= 10:  # 5+ bytes
                raw = decode_frame(hex_data, 3, 16)
                latest["turbine_rpm"] = raw - 2000

            # Emit a row when all three signals have been seen at least once
            if all(v is not None for v in latest.values()):
                eng = latest["engine_rpm"]
                tur = latest["turbine_rpm"]
                slip_ratio = round(eng / tur, 4) if tur and tur > 0 else None
                rows.append(
                    (
                        ts,
                        latest["reinf"],
